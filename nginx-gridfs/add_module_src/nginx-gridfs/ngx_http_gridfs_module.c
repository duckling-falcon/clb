...
/* Parse config directive */
static char* ngx_http_gridfs(ngx_conf_t* directive, ngx_command_t* command,
		void* gridfs_conf);
...
static char* ngx_http_gridfs_merge_loc_conf(ngx_conf_t* directive, void* parent,
		void* child);
...
static void fill_filename(const char* filename, ngx_str_t* value,
	...
	int size = strlen(template) + strlen(filename) - 2;
	ngx_buf_t *b;
	b = ngx_create_temp_buf(request->pool, size + 1);
	b->last = ngx_sprintf(b->last, template, filename);
...
static ngx_int_t ngx_http_gridfs_handler(ngx_http_request_t* request) {
	...
	if(!is_no_filename_type(contenttype)){
