...
{ ngx_string("mongo"), NGX_HTTP_LOC_CONF | NGX_CONF_1MORE, ngx_http_mongo,
        NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },

{ ngx_string("gridfs"), NGX_HTTP_LOC_CONF | NGX_CONF_1MORE, ngx_http_gridfs,
        NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },

ngx_null_command };

/* Module context. */
static ngx_http_module_t ngx_http_gridfs_module_ctx = { NULL, /* preconfiguration */
NULL, /* postconfiguration */
ngx_http_gridfs_create_main_conf, NULL, /* init main configuration */
NULL, /* create server configuration */
NULL, /* init serever configuration */
ngx_http_gridfs_create_loc_conf, ngx_http_gridfs_merge_loc_conf };

/* Module definition. */
ngx_module_t ngx_http_gridfs_module = { NGX_MODULE_V1,
        &ngx_http_gridfs_module_ctx, ngx_http_gridfs_commands, NGX_HTTP_MODULE,
        NULL, NULL, ngx_http_gridfs_init_worker, NULL, NULL, NULL, NULL,
        NGX_MODULE_V1_PADDING };

ngx_array_t ngx_http_mongo_connections;

/* Parse the 'mongo' directive. */
static char * ngx_http_mongo(ngx_conf_t *cf, ngx_command_t *cmd,
        void *void_conf) {
    ngx_str_t *value;
    ngx_url_t u;
    ngx_uint_t i;
    ...
    for (i = start; i < cf->args->nelts; i++) {
        ...
        if (ngx_parse_url(cf->pool, &u) != NGX_OK) {
            if (u.err) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in mongo \"%V\"",
                        u.err, &u.url);
            }
...
static char* ngx_http_gridfs(ngx_conf_t* cf, ngx_command_t* command,
        void* void_conf) {
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        if (ngx_strncmp(value[i].data, "root_collection=", 16) == 0) {
            gridfs_loc_conf->root_collection.data =
                    (u_char *) &value[i].data[16];
            gridfs_loc_conf->root_collection.len =
                    ngx_strlen(&value[i].data[16]);
            continue;
        }
        ...
        if (ngx_strncmp(value[i].data, "field=", 6) == 0) {
            ...
            /* Currently only support for "_id" and "filename" and "storageKey" */
            if (gridfs_loc_conf->field.data != NULL
                    ...
                    && ngx_strcmp(gridfs_loc_conf->field.data, "storageKey")
                            != 0) {
        ...
        if (ngx_strncmp(value[i].data, "type=", 5) == 0) {
            ...
            if (type.len == 0) {
                ...
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Unsupported Type: %s",
                        (char *) value[i].data);
                return NGX_CONF_ERROR ;
            }
    ...
    if ((gridfs_loc_conf->user.data == NULL || gridfs_loc_conf->user.len == 0)
            && !(gridfs_loc_conf->pass.data == NULL
                    || gridfs_loc_conf->pass.len == 0)) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Password without username");
        return NGX_CONF_ERROR ;
    }
    ...
    if (!(gridfs_loc_conf->user.data == NULL || gridfs_loc_conf->user.len == 0)
            && (gridfs_loc_conf->pass.data == NULL
                    || gridfs_loc_conf->pass.len == 0)) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Username without password");
        return NGX_CONF_ERROR ;
    }
...
static void *ngx_http_gridfs_create_main_conf(ngx_conf_t *cf) {
    ...
    gridfs_main_conf = ngx_pcalloc(cf->pool,
            sizeof(ngx_http_gridfs_main_conf_t));
    if (gridfs_main_conf == NULL ) {
        return NULL ;
    }

    if (ngx_array_init(&gridfs_main_conf->loc_confs, cf->pool, 4,
            sizeof(ngx_http_gridfs_loc_conf_t *)) != NGX_OK) {
        return NULL ;
    }
...
static void* ngx_http_gridfs_create_loc_conf(ngx_conf_t* directive) {
    ...
    gridfs_conf = ngx_pcalloc(directive->pool,
...
static char* ngx_http_gridfs_merge_loc_conf(ngx_conf_t* cf, void* void_parent,
        void* void_child) {
    ngx_http_gridfs_loc_conf_t *parent = void_parent;
    ngx_http_gridfs_loc_conf_t *child = void_child;
    ngx_http_gridfs_main_conf_t *gridfs_main_conf =
            ngx_http_conf_get_module_main_conf(cf, ngx_http_gridfs_module) ;
    ngx_http_gridfs_loc_conf_t **gridfs_loc_conf;
    ngx_http_mongod_server_t *mongod_server;

    ngx_conf_merge_str_value(child->db, parent->db, NULL);
    ngx_conf_merge_str_value(child->root_collection, parent->root_collection,
            "fs");
...
ngx_http_mongo_connection_t* ngx_http_get_mongo_connection(ngx_str_t name) {
    ...
    for (i = 0; i < ngx_http_mongo_connections.nelts; i++) {
        if (name.len == mongo_conns[i].name.len
                && ngx_strncmp(name.data, mongo_conns[i].name.data, name.len)
                        == 0) {
...
static ngx_int_t ngx_http_mongo_authenticate(ngx_log_t *log,
        ngx_http_gridfs_loc_conf_t *gridfs_loc_conf) {
    ...
    if (mongo_conn == NULL ) {
        ngx_log_error(NGX_LOG_ERR, log, 0, "Mongo Connection not found: \"%V\"",
                &gridfs_loc_conf->mongo);
    }

    // Authenticate
    if (gridfs_loc_conf->user.data != NULL
            && gridfs_loc_conf->pass.data != NULL ) {
        if (mongo_cmd_authenticate(&mongo_conn->conn,
                (const char*) gridfs_loc_conf->db.data,
                (const char*) gridfs_loc_conf->user.data,
                (const char*) gridfs_loc_conf->pass.data) != MONGO_OK) {
            ngx_log_error(NGX_LOG_ERR, log, 0, "Invalid mongo user/pass: %s/%s",
                    gridfs_loc_conf->user.data, gridfs_loc_conf->pass.data);
            return NGX_ERROR;
        }
    ...
    ngx_cpystrn((u_char*) test, (u_char*) gridfs_loc_conf->db.data,
            gridfs_loc_conf->db.len + 1);
    ngx_cpystrn((u_char*) (test + gridfs_loc_conf->db.len), (u_char*) ".test",
            sizeof(".test"));
    bson_empty(&empty);
    cursor = mongo_find(&mongo_conn->conn, test, &empty, NULL, 0, 0, 0);
    error = mongo_cmd_get_last_error(&mongo_conn->conn,
            (char*) gridfs_loc_conf->db.data, NULL );
    free(test);
...
static ngx_int_t ngx_http_mongo_add_connection(ngx_cycle_t* cycle,
        ngx_http_gridfs_loc_conf_t* gridfs_loc_conf) {
    ngx_http_mongo_connection_t* mongo_conn;
    int status;
    ...
    mongo_conn->auths = ngx_array_create(cycle->pool, 4,
            sizeof(ngx_http_mongo_auth_t));

    if (gridfs_loc_conf->mongods->nelts == 1) {
        ngx_cpystrn(host, mongods[0].host.data, mongods[0].host.len + 1);
        status = mongo_connect(&mongo_conn->conn, (const char*) host,
                mongods[0].port);
    } else if (gridfs_loc_conf->mongods->nelts >= 2
            && gridfs_loc_conf->mongods->nelts < 9) {

        /* Initiate replica set connection. */
        mongo_replset_init(&mongo_conn->conn,
                (const char *) gridfs_loc_conf->replset.data);

        /* Add replica set seeds. */
        for (i = 0; i < gridfs_loc_conf->mongods->nelts; ++i) {
            ngx_cpystrn(host, mongods[i].host.data, mongods[i].host.len + 1);
            mongo_replset_add_seed(&mongo_conn->conn, (const char *) host,
                    mongods[i].port);
        }
    ...
    case MONGO_CONN_NO_SOCKET:
        ngx_log_error(NGX_LOG_ERR, cycle->log, 0, "Mongo Exception: No Socket");
        return NGX_ERROR;
    ...
    case MONGO_CONN_BAD_SET_NAME:
        ngx_log_error(NGX_LOG_ERR, cycle->log, 0,
                "Mongo Exception: Replica set name %s does not match.",
                gridfs_loc_conf->replset.data);
        return NGX_ERROR;
...
static ngx_int_t ngx_http_gridfs_init_worker(ngx_cycle_t* cycle) {
    ngx_http_gridfs_main_conf_t* gridfs_main_conf =
            ngx_http_cycle_get_module_main_conf(cycle, ngx_http_gridfs_module);
...
static ngx_int_t ngx_http_mongo_reconnect(ngx_log_t *log,
        ngx_http_mongo_connection_t *mongo_conn) {
    ...
    case MONGO_CONN_SUCCESS:
        break;
    case MONGO_CONN_NO_SOCKET:
        ngx_log_error(NGX_LOG_ERR, log, 0, "Mongo Exception: No Socket");
        return NGX_ERROR;
    case MONGO_CONN_FAIL:
        ngx_log_error(NGX_LOG_ERR, log, 0,
                "Mongo Exception: Connection Failure %s:%i;",
                mongo_conn->conn.primary->host, mongo_conn->conn.primary->port);
        return NGX_ERROR;
    case MONGO_CONN_ADDR_FAIL:
        ngx_log_error(NGX_LOG_ERR, log, 0,
                "Mongo Exception: getaddrinfo Failure");
        return NGX_ERROR;
    case MONGO_CONN_NOT_MASTER:
        ngx_log_error(NGX_LOG_ERR, log, 0, "Mongo Exception: Not Master");
        return NGX_ERROR;
    default:
        ngx_log_error(NGX_LOG_ERR, log, 0, "Mongo Exception: Unknown Error");
        return NGX_ERROR;
    }
...
static ngx_int_t ngx_http_mongo_reauth(ngx_log_t *log,
        ngx_http_mongo_connection_t *mongo_conn) {
    ...
    for (i = 0; i < mongo_conn->auths->nelts; i++) {
        ...
        if (status != MONGO_OK) {
            ngx_log_error(NGX_LOG_ERR, log, 0,
                    ...
                    auths[i].user.data, auths[i].pass.data);
            return NGX_ERROR;
        }
    }
...
static int htoi(char* h) {
    ...
    if (ngx_strchr(ok, h[0]) == NULL || ngx_strchr(ok,h[1]) == NULL ) {
        return -1;
    }
...
//=========add by clive:start==========
static void fill_filename(const char* filename, ngx_str_t* value,
        ngx_http_request_t* r) {
    u_char* p = NULL;
    if(r->headers_in.msie == 1 || r->headers_in.msie6 == 1){
        int raw_len = ngx_strlen(filename);
        int max_len = raw_len * 9;
        p = ngx_pnalloc(r->pool,max_len);
        int i = 0;
        for(i=0;i<max_len;i++)
            *(p+i) = '\0';
        ngx_escape_uri(p,(u_char *)filename,raw_len,NGX_ESCAPE_URI);
    }else if(r->headers_in.chrome == 1 || r->headers_in.safari == 1){
        p = (u_char *)filename;
    }else{
        p = (u_char *)filename;
    }
    char* template = "attachment; filename=\"%s\"";
    int size = ngx_strlen(template) + ngx_strlen(p) - 2;
    ngx_buf_t *b;
    b = ngx_create_temp_buf(r->pool, size + 1);
    b->last = ngx_sprintf(b->last, template, p);
    value->data = (u_char*) b->start;
    value->len = size;
}



static int contains_cotent_disposition(ngx_http_request_t * request){
    ngx_table_elt_t* h = NULL;
    ngx_list_part_t* current = &(request->headers_in.headers.part);
    int result = 0;
    unsigned int i = 0;
    h = (ngx_table_elt_t*)current->elts;
     ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,"Header: %s",h[i].key.data);
    for(i=0;;i++){
        if(i >= current->nelts){
            ...
            h = (ngx_table_elt_t*)current->nelts;
            i = 0;
        }
        ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,"Header: %s",h[i].key.data);
        if(ngx_strcmp(h[i].key.data,"Content-Dispostion") == 0){
            result = 1;
            break;
        }   
    }
...
/*static void fill_filename(const char* filename, ngx_str_t* value,
    ...
    int _fn_len = ngx_strlen(filename);
        ngx_buf_t* b1 = ngx_create_temp_buf(request->pool, _fn_len*9);
    u_char* p = (u_char *)ngx_escape_uri(b1->last,(u_char *)filename,_fn_len,NGX_ESCAPE_URI_COMPONENT);
    char* template = "attachment; filename=\"%s\"";
    int size = ngx_strlen(template) + ngx_strlen(p) - 2;
    ngx_buf_t *b;
    b = ngx_create_temp_buf(request->pool, size + 1);
    b->last = ngx_sprintf(b->last, template, p);
    value->data = (u_char*) b->start;
    value->len = size;
}*/

/*static int get_uint64_size(uint64_t num){
    int len = 0;
    uint64_t a = num;
    if (a==0){
         len = 1;
...
/*
static char * convert_to_str(uint64_t num,int len){
    if(num==0)
        return "0";
    uint64_t a = num;
    char* ptr = NULL;
...
/*static void fill_contentlength(const uint64_t length,ngx_str_t* value,ngx_http_request_t* request){
    ngx_buf_t *b;
    int size = get_uint64_size(length);
//  char* temp = convert_to_str(length,size);
    b = ngx_create_temp_buf(request->pool, size + 1);
    b->last = ngx_sprintf(b->last, "%d", length);
    b->start[size]='\0';
    value->len = size;
    value->data = (u_char*)b->start;
}*/
...
const char* NO_FILENAME_TYPES[2] = {"text/css","application/x-shockwave-flash"};
static int is_no_filename_type(const char * contenttype){
    int i = 0;
    for(i=0;i<2;i++){
        if(strcmp(contenttype,NO_FILENAME_TYPES[i])==0){
            return 1;
        }
    }
    return 0;
...
static ngx_int_t ngx_http_gridfs_handler(ngx_http_request_t* request) {
    ...
    if (!(&mongo_conn->conn.connected)
            && (ngx_http_mongo_reconnect(request->connection->log, mongo_conn)
                    == NGX_ERROR
                    || ngx_http_mongo_reauth(request->connection->log,
                            mongo_conn) == NGX_ERROR)) {
        ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,
    ...
    value = (char*) malloc(
            sizeof(char) * (full_uri.len - location_name.len + 1));
    if (value == NULL ) {
        ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,
                "Failed to allocate memory for value buffer.");
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
    memcpy(value, full_uri.data + location_name.len,
            full_uri.len - location_name.len);
    ...
    do {
        e = FALSE;
        if (gridfs_init(&mongo_conn->conn, (const char*) gridfs_conf->db.data,
                (const char*) gridfs_conf->root_collection.data,
                &gfs) != MONGO_OK) {
            e = TRUE;
            ecounter++;
            if (ecounter > MONGO_MAX_RETRIES_PER_REQUEST
                    || ngx_http_mongo_reconnect(request->connection->log,
                            mongo_conn) == NGX_ERROR
                    || ngx_http_mongo_reauth(request->connection->log,
                            mongo_conn) == NGX_ERROR) {
                ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,
    ...
    case BSON_OID:
        ...
        break;
    case BSON_INT:
        bson_append_int(&query, (char*) gridfs_conf->field.data,
                ngx_atoi((u_char*) value, strlen(value)));
        break;
    case BSON_STRING:
        //=========add by clive:start ====when config is filename , use filename and spaceName to search ==========
        arg_fnm = strchr(value, '/'); //fetch filename value from uri
        spn_len = (int) (arg_fnm - value);
        arg_spn = (char *) malloc(sizeof(char) * spn_len + 1); //get spacename from uri
        int k = 0;
        for (k = 0; k < spn_len; k++) {
            arg_spn[k] = value[k];
        }
        arg_spn[k] = '\0';
        arg_fnm = arg_fnm + 1;
        bson_oid_from_string(&oid, arg_spn);
        bson_append_oid(&query, "spaceName", &oid);
        bson_append_string(&query, "filename", arg_fnm);
        free(arg_spn);
        //=========add by clive:end====when config is filename , use filename and spaceName to search ==========
    ...
    //=========add filename to response header by clive:start==========
    filename = (char*) gridfile_get_filename(&gfile);
    //is_pub = (bson_string_t) gridfile_get_field(&gfile, "isPub");
    //if (strcmp(is_pub->content, "false") == 0) {
    //  ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,
    //          "access forbidden for private document");
    //  return NGX_HTTP_FORBIDDEN;
    //}
    if(!is_no_filename_type(contenttype) && !contains_cotent_disposition(request)){
        h = ngx_list_push(&request->headers_out.headers);
        if (h == NULL ) {
            return NGX_ERROR;
        }
        h->hash = 1;
        h->key.len = sizeof("Content-Disposition") - 1;
        h->key.data = (u_char *) "Content-Disposition";
        fill_filename(filename, &(h->value), request);
    }
    //=========add filename to response header by clive:end  ==========

    request->headers_out.status = NGX_HTTP_OK;
    request->headers_out.content_length_n = length;
    //=========add content-length header to response header by clive:start==========
/*  request->headers_out.content_length = ngx_list_push(&request->headers_out.headers);
    request->headers_out.content_length->hash = 1;
    request->headers_out.content_length->key.len = sizeof("Content-Length") - 1;
    request->headers_out.content_length->key.data = (u_char*) "Content-Length";
    fill_contentlength(length,&(request->headers_out.content_length->value),request); */
    //=========add content-length header to response header by clive:end  ==========
    if (contenttype != NULL ) {
    ...
    if (md5 != NULL ) {
        request->headers_out.etag = ngx_list_push(
    ...
    if (last_modified) {
        request->headers_out.last_modified_time =
                (time_t) (last_modified / 1000);
    }

    /* Determine if content is gzipped, set headers accordingly */
    if (gridfile_get_boolean(&gfile, "gzipped")) {
        ngx_log_error(NGX_LOG_ERR, request->connection->log, 0,
                gridfile_get_field(&gfile, "gzipped"));
        request->headers_out.content_encoding = ngx_list_push(
                &request->headers_out.headers);
        ...
        request->headers_out.content_encoding->key.len =
                sizeof("Content-Encoding") - 1;
        request->headers_out.content_encoding->key.data =
    ...
    ngx_int_t hrc = ngx_http_send_header(request);
    if (hrc == NGX_ERROR || hrc > NGX_OK || request->header_only){
        return hrc;
    };
    ...
    cursors = (mongo_cursor **) ngx_pcalloc(request->pool,
            sizeof(mongo_cursor *) * numchunks);
    if (cursors == NULL ) {
        gridfile_destroy(&gfile);
        gridfs_destroy(&gfs);
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    ...
    gridfs_cln = ngx_pool_cleanup_add(request->pool,
            sizeof(ngx_http_gridfs_cleanup_t));
    if (gridfs_cln == NULL ) {
        gridfile_destroy(&gfile);
        gridfs_destroy(&gfs);
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
