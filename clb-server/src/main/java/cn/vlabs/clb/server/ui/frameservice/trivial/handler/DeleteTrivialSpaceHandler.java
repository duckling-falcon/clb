...
public class DeleteTrivialSpaceHandler extends CLBAbstractAction {
    ...
    protected Object doAction(Object arg) throws ServiceException, BaseException {
        try {
            ...
            TrivialItem itm = tf.getTrivialItemByStorageKey(spaceName);
            DocMeta meta = df.getDocMetaByDocid(itm.getAppid(), itm.getDocid());
            af.checkPermission(meta);
            tf.removeTrivialFiles(itm.getSpaceName());
            return tf.removeTrivialItem(itm) > 0;
