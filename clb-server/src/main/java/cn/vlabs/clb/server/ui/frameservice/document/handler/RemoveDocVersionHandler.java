...
public class RemoveDocVersionHandler extends CLBAbstractAction {

    private DocumentFacade df = AppFacade.getDocumentFacade();
    private AuthFacade af = AppFacade.getAuthFacade();

    private static final Logger LOG = Logger.getLogger(RemoveDocVersionHandler.class);
    ...
    protected Object doAction(Object arg) throws ServiceException, BaseException {
        try {
            ...
            LOG.info(String.format("Appid=%d want to delete doc version where is docid=%d and version=%d",
                    appid, dv.getDocid(), dv.getVersion()));
            return df.removeDocVersion(dv);
